...
uint8_t buffer[1024], buffer2[400], buffer3[80];
...
void gpsTask(void *pData)
{
    ...
    while (1)
    {
        if (isGpsOn)
        {
            ...
            convertTime(time(NULL) + 7200);
            snprintf(buffer2, sizeof(buffer2), "%s%.0f m\nFix=%d %d %d\nSat=%d %d\nBatt=%d%% %.3fV\nhttps://www.google.com/maps/search/%f,%f\r\n", buffer3, gpsInfo->gga.altitude, gpsInfo->gsa[0].fix_type, gpsInfo->gsa[1].fix_type, gpsInfo->gga.fix_quality, gpsInfo->gga.satellites_tracked, gpsInfo->gsv[0].total_sats, percent, v*0.001f,latitude, longitude);
            SendSMS(buffer2);
            Trace(1, "tutos request: %s", buffer2);
        }
